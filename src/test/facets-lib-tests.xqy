...
declare function test-render-selected-facets() 
{
  ...
  let $t8 := "xxx:""yyy""" (: invalid facet name and value :)
  let $t9 := "blah pubtype:book blah di blah" (: unquoted facet value -  not acceptable:)
  
  let $r9 := try { f:render-selected-facets($t8) } catch ($ex) { $ex }
...
(:   ,assert:empty(f:render-selected-facets($t9), "unquote facet value: not acceptable 'blah pubtype:book bla'"):)
   ,assert:error($r9, "PI-INVALIDFACETTYPE", "invalid facet name and value: 'xxx:""yyy"" blah di blah'")
   ,assert:equal($r9/error:data/fn:string(), 'The value "xxx" is not a valid facet type.', "Make sure that error message corresponds")
